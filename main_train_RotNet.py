...
parser = argparse.ArgumentParser(description='Training Rot_Net')
...
def test_network(dataset, set_name, network, print_output=False):
    ...
    if isinstance(print_output, str):
        ...
        print_path = './models_rot_net/dump'
    else:
        print_path = None
...
def train_network():
    ...
    for e_i, epoch in enumerate(range(start_epoch, num_epochs)):
        ...
        for b_i in range(num_batches_per_epoch):
            ...
            Xs, Ys, _ = dataset.train_get_data(batch_size=args.batch_size)
    ...
    with open(os.path.join(snapshot_path, 'best_model.pkl'), 'wb') as unsup_file:
